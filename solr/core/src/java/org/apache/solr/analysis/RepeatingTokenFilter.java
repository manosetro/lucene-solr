    ...
    public final boolean incrementToken() throws IOException {
        if (repeatCount == -1) {
            if (input.incrementToken()) {
                ...
                for (int i = 0; i < length; i++) {
                    if (buffer[i] == delimiter) {
                        ...
                        if (repeatCount > 0) {
                            ...
                            SolrCore.requestLog.warn("0 relevance value found in term value: " + termAtt.toString());
                            termAtt.setLength(i); // truncate "term" to remove all the relevance values
                            return false;
                        } else {
                            repeatCount = -1;
                ...
                SolrCore.requestLog.error("failed to get payload value from term: " + termAtt.toString());
                return false;
            } else {
                // increment token chain failed
                SolrCore.requestLog.warn("increment token chain failed");
                return false;
            }
        } else if (repeatCount-- > 0) {
            return true;
